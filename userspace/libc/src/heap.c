...
/** Initialized heap.
 *
 * Uses pointer to app end and size of the available memory to determine
...
static void heap_init(void) {
    ...
    uintptr_t start_ptr = align(debug_get_app_endptr(), MIN_ALLOCATION_SIZE);
...
static inline void compact(link_t* prev, link_t* next) {
    block_header_t* prev_header = VALID_HEADER_LINK(prev) ? HEADER_FROM_LINK(prev) : NULL;
    block_header_t* next_header = VALID_HEADER_LINK(next) ? HEADER_FROM_LINK(next) : NULL;
